...
from rpi_d3m_primitives.featSelect.helperFunctions import Crosstab_Parse, fixed_point
...
def mi_pseudoBayesian(X, Y, *args):
    ...
    px = np.transpose(np.tile(np.sum(joint_table, 1), (hm_y, 1)))
    py = np.tile(np.sum(joint_table, 0), (hm_x, 1))
#    print(px)
...
def mi_Bayesian( X, Y, *args):
    ...
    Nx = np.transpose(np.tile(np.sum(Nxy, 1), (hm_y, 1)))
    Ny = np.tile(np.sum(Nxy, 0), (hm_x, 1))
...
def mi_Full_Bayesian_fixPt(X, Y, *args):
    if len(args) == 0:
        hm_x = len(np.unique(X))
        hm_y = len(np.unique(Y))
    else:
...
def MutualInfo_funs( X, Y, method, *args):
    if len(args) == 0:
        m = len(np.unique(X))
        n = len(np.unique(Y))
    else:
    ...
    if method == 'counting':
        mutu_info = mi(X, Y)
    elif method == 'pseudoBayesian':
        mutu_info = mi_pseudoBayesian(X, Y, m, n)
    elif method == 'Bayesian':
        mutu_info = mi_Bayesian(X, Y, m, n)
    elif method == 'fullBayesian':
        mutu_info, _ = mi_Full_Bayesian_fixPt(X, Y, m, n)
